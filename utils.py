...
class FeatureExtractorVGG19(nn.Module):
    def __init__(self, architecture: Sequence[int], activations: Sequence[int]):
        ...
        self.model.classifier = DeepNeuralNetwork(
            1280,
            architecture,
            activations
        )
...
class ContrastiveLearning(Module):
    def __init__(self, model1: nn.Module, **hparams) -> None:
        ...
        self.model2 = FeatureExtractorVGG19(
            self.architecture,
            self.activations
        )

class NonLinearConstrastiveLearning(ContrastiveLearning):
    def __init__(self, hidden_size: int, num_layers: int, dropout: float, bidirectional: bool, architecture: Sequence[int], activations: Sequence[int]) -> None:
        super().__init__(nn.LSTM(7, hidden_size, num_layers, batch_first = True, dropout = dropout, bidirectional=bidirectional), architecture, activations)
...
class CosmicModule(LightningDataModule):
    def __init__(self, dataset: CosmicRayDataset, batch_size: int, pin_memory: bool = True, num_workers: int = 12) -> None:
